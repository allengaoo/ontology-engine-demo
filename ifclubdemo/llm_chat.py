...
.env（democode/.env，与 phase6 llm_coder 相同）：
  ...
  LLM_MODEL      （可选；端侧默认 Qwen2.5-Coder-32B-Instruct，非 32B 会被自动降级）

端侧小模型策略：默认 Qwen3-Coder-30B-A3B-Instruct（DashScope id）。
若 .env 配置了更大模型，默认强制回退；设 DEMOCODE_EDGE_MODEL=0 可关闭。
"""
...
# DashScope OpenAI 兼容接口使用小写 id
# 默认：Qwen3-Coder-30B-A3B-Instruct（本账号已开通；原 2.5-Coder-32B 常 404）
DEFAULT_LLM_MODEL = "qwen3-coder-30b-a3b-instruct"
DEFAULT_LLM_BASE_URL = "https://dashscope.aliyuncs.com/compatible-mode/v1"

# 显式允许的端侧模型（小写比较；约 ≤32B / MoE-A3B 级）
...
def resolve_llm_model(*, warn: bool = True) -> str:
    """
    解析实际使用的模型名。端侧 demo 默认锁定 Qwen3-Coder-30B-A3B-Instruct。

    - 未设置 LLM_MODEL → 默认 Coder-30B-A3B
    - 已设置且为允许列表 / 名称含 30b|32b → 使用该值
    - 已设置为更大模型 → 强制回退到默认并告警
    """
    _load_dotenv_once()
    ...
    if (
        normalized in ALLOWED_EDGE_MODELS
        or "30b" in normalized
        or "32b" in normalized
    ):
        # DashScope 通常要小写 id
        return normalized
    ...
    if _edge_model_enforced():
        if warn:
            msg = (
                f"LLM_MODEL={configured!r} 非端侧小模型，"
