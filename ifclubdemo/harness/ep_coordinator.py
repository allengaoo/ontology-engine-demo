...
IFCLUB_ROOT = Path(__file__).parent.parent
...
from agents.coding_agent import (  # noqa: E402
    CodingAgent,
    UnitDiff,
    summarize_diff_for_context,
)
from agents.structure_plan import PlanUnit, StructurePlan, UnitKind  # noqa: E402
...
from harness.atomicity_check import (  # noqa: E402
    AtomicityCheck,
    CheckOutcome,
...
class EPCoordinator:
    ...
    def __init__(
        ...
        *,
        ...
        self.workspace_root = Path(
            workspace_root or (IFCLUB_ROOT / "workspace" / "app")
        ).resolve()
        self.apply_enabled = apply_enabled
        self.intent_router = IntentRouter()
        self.fed_injector = FederatedInjectorWrapper(fed_graph)
        extra_forbid = ()
        if self.workspace_root.name == "meeting_order":
            extra_forbid = MEETING_FORBIDDEN_SUBSTRINGS
        self.atomicity = AtomicityCheck(
            allowed_prefixes=allowed_path_prefixes,
            extra_forbidden_substrings=extra_forbid,
        )
        ...
        self.state_store = DagStateStore(state_dir or IFCLUB_ROOT / "workspace" / "ep_state")
    ...
    def run_ep(
        ...
        def _idle_ops() -> None:
            """单 EP 结束（成功/失败）后的空闲 GC；EpQueue 若自带 ops 可不再挂本字段。"""
            if self.memory_ops is None:
                return
            try:
                ops = self.memory_ops.run_after_ep_idle()
                print(ops.summary())
            except Exception as exc:  # noqa: BLE001 — 治理失败不阻断 EP
                print(f"  ⚠ queue_idle_gc 失败: {exc}")
        ...
        # 同步 freeze 到 Atomicity / DiffApplier
        try:
            from harness.freeze_state import load_frozen_prefixes

            frozen = list(
                task.context.get("_frozen_prefixes")
                or load_frozen_prefixes(self.workspace_root)
            )
            task.context["_frozen_prefixes"] = frozen
            self.atomicity.set_frozen_prefixes(frozen)
            self.applier.set_frozen_prefixes(frozen)
        except Exception as exc:  # noqa: BLE001
            print(f"  ⚠ freeze sync 失败: {exc}")
        ...
        while struct_retry <= self.MAX_STRUCT_RETRY:
            ...
            if bsa_out.status != "completed" or not isinstance(bsa_out.output, dict):
                ...
                result.turns.append(EPTurnRecord(
                    ...
                    agent_name="BusinessStructureAgent",
                    outcome="failed",
                    detail=str((bsa_out.output or {}).get("error", bsa_out.status)),
                ))
                return result
            structure_plan = self._dict_to_plan(bsa_out.output)

            # 计划期细化：剔除 freeze + Unit 预算
            from harness.plan_refine import refine_structure_plan

            max_units = int(task.context.get("_max_units") or 2)
            refine = refine_structure_plan(
                structure_plan,
                frozen_prefixes=task.context.get("_frozen_prefixes") or [],
                max_units=max_units,
            )
            structure_plan = refine.plan
            if not refine.ok:
                result.turns.append(EPTurnRecord(
                    phase=EPPhase.PLAN,
                    agent_name="PlanRefine",
                    outcome=CheckOutcome.FAIL_STRUCT.value,
                    rule_id=refine.rule_id,
                ...
                if struct_retry > self.MAX_STRUCT_RETRY:
                    ...
                    self._save_checkpoint(
                        ep_id, task, "plan", struct_retry, impl_retry,
                        completed_units, structure_plan,
                    )
                    return result
                print(
                    f"\n[Harness] PlanRefine: FAIL_STRUCT rule={refine.rule_id} "
                    f"{refine.detail}"
                )
                print(f"  → FAIL_STRUCT，BSA replan {struct_retry}/{self.MAX_STRUCT_RETRY}")
                task.context["_struct_feedback"] = (
                    f"{refine.detail}; frozen={task.context.get('_frozen_prefixes')}; "
                    f"max_units={max_units}; 禁止改 freeze 路径；每轮≤{max_units} 个文件"
                )
                completed_units = []
                impl_retry = 0
                continue
            ...
            # 上游签名摘要（只读）注入，供 CA 对齐；不占 EP 轮次
            task.context["_upstream_signatures"] = self._build_upstream_signatures(task)
            ...
            while impl_retry <= self.MAX_IMPL_RETRY:
                if impl_retry > 0:
                    ...
                    if injected:
                        self.bg_store.flush()
                        print(f"  [bg_store] impl-retry 注入 {injected} 条失败反馈")

                scratch_dir = self.applier.scratch_root / ep_id
                task.context["_scratch_root"] = (
                    str(scratch_dir) if scratch_dir.exists() else ""
                )
                # retry 时刷新签名（磁盘可能被回滚）
                task.context["_upstream_signatures"] = self._build_upstream_signatures(task)
                ...
                # 每轮重试重新生成全部 Unit（带上下文），不沿用半成品 completed
                completed_units = []
                ...
                for unit in units:
                    ...
                    task.context["_generated_so_far"] = list(generated_so_far)
                    try:
                        diff = self.ca.execute_unit(
                            unit, task, action=structure_plan.action
                        )
                    except RuntimeError as exc:
                        print(f"  ✗ CodingAgent 失败: {exc}")
                    ...
                    generated_so_far.append(summarize_diff_for_context(diff))
                ...
                if verify.outcome == VerifyOutcome.PASS and self.apply_enabled and not dry_run:
                    ...
                    if not apply_report.ok:
                        ...
                        if verify.outcome != VerifyOutcome.PASS:
                            scratch = self.applier.save_scratch(
                                all_diffs, ep_id=ep_id
                            )
                            print(f"  [DiffApplier] 失败现场 → scratch {scratch}")
                ...
                feedback = {
                    ...
                    "command_output": getattr(verify, "command_output", "")[:3500],
                }
                task.context["_last_verify_feedback"] = feedback
    ...
    def _build_upstream_signatures(self, task: Task) -> str:
        """从 workspace 抽取上游文件签名摘要，供 CA 对齐。"""
        from agents.code_context import extract_exports_summary
        from harness.plan_refine import collect_upstream_signature_paths

        root = Path(task.context.get("_workspace_root") or self.workspace_root)
        app = root.name if root else ""
        parts: List[str] = []
        for rel in collect_upstream_signature_paths(app=app):
            path = root / rel
            if not path.is_file():
                continue
            try:
            ...
            if rel.endswith(".json"):
                ...
                summary = extract_exports_summary(text, max_chars=900)
            parts.append(f"### {rel}\n{summary}")
        if not parts:
            return "（尚无上游文件）"
        blob = "\n\n".join(parts)
        print(f"  [Harness] 上游签名注入 {len(parts)} 个文件 app={app or '?'}")
        return blob
