    ...
    *,
    ...
    if bad_dirs:
        ...
        result.detail = (
            ...
            f"{bad_dirs}；例如用 shift.py + roster.py，不要用 models/"
        )
        return result
    ...
    if len(plan.units) > max_units:
        ...
        result.detail = (
            ...
            f"（当前: {paths}）。本轮若要做 shift+roster，恰好 2 个文件路径即可。"
        )
        return result
...
def collect_upstream_signature_paths(
    ...
    *,
    ...
    if not app_name:
        if "meeting_order" in rel:
            app_name = "meeting_order"
        else:
            app_name = "oncall"

    if app_name == "meeting_order":
        base = [
    ...
    base = [
        ...
        "backend/src/oncall/models/__init__.py",
        "backend/src/oncall/domain/rules.py",
        "backend/src/oncall/domain/scheduler.py",
        "backend/src/oncall/db.py",
        "backend/src/oncall/main.py",
        "docs/oncall_schema.json",
    ]
    if "api/" in rel or "test_api" in rel:
        base.extend(
            [
                "backend/src/oncall/api/engineers.py",
                "backend/src/oncall/api/roster.py",
            ]
        )
    return base
