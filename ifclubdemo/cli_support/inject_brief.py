"""
从业务说明 Markdown 生成领域记忆实例（Constraint / Pattern / AntiPattern）。
"""
...
SECTION_MAP = {
    "硬约束": ("ConstraintMemory", "CN", "reject", "CROSS_CUTTING"),
    "推荐模式": ("PatternMemory", "PAT", "", "DOMAIN"),
...
def inject_business_brief(
    ...
    *,
    ...
    for part in memory_dir.resolve().parts:
        ...
        # memory_dir ≈ workspace/<app>/.ontology_agent/memory
        try:
            app_slug = memory_dir.resolve().parents[1].name.upper().replace("-", "_")
        except IndexError:
            app_slug = "APP"
    ...
    for section_name, bullets in sections.items():
        ...
        for bullet in bullets:
            counters[prefix] += 1
            mid = f"{prefix}-{app_slug}-{counters[prefix]:03d}"
            rel = f"{layer}/{mid}.md"
    ...
    if not dry_run:
        ...
        report_path = memory_dir.parent / "inject_report.json"
